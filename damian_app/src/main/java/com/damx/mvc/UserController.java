...
public class UserController {
    ...
    @RequestMapping(value="/showCurrentUser", method = RequestMethod.POST)
    public @ResponseBody User showCurrentUser(@RequestParam String token)
    {
        int userid = getIdByToken(token);
        if (userid == -1) // according to getIdByToken, -1 means invalid token
            return null;
